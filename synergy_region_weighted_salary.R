...
filter_synergy <- function(css) {
  css <- css %>% 
    select(everything()) %>% 
    filter(Organisation %in% defra_orgs | Organisation %in% dwp_orgs | Organisation %in% ho_orgs | Organisation %in% moj_orgs)
}
...
completedata <<- completedata %>%
  mutate(ln_Region_Weighted_Average_Salary = ifelse(Cluster=='Synergy' & Num==observation, region_weighted_average, ln_Region_Weighted_Average_Salary))
...
x <- c(1:29)
y <- completedata$ln_Region_Weighted_Average_Salary[31:59]
print(y)
...
completedata <- completedata %>%
  mutate(ln_Region_Weighted_Average_Salary_ncsi = ifelse(Cluster == 'Synergy', NA, ln_Region_Weighted_Average_Salary_ncsi))

completedata$ln_Region_Weighted_Average_Salary_ncsi[synergy_indices] <- spline_values
...
write_xlsx(completedata, "outputs/results_synergy_region_weights.xlsx")
