...
filter_defence <- function(css) {
  css <- css %>% 
    select(everything()) %>% 
    filter(Organisation %in% deqsu_orgs | Organisation %in% dstl_orgs | Organisation %in% mod_orgs)
}
...
completedata <<- completedata %>%
  mutate(ln_Region_Weighted_Average_Salary = ifelse(Cluster=='Defence' & Num==observation, region_weighted_average, ln_Region_Weighted_Average_Salary))
...
x <- c(1:29)
y <- completedata$ln_Region_Weighted_Average_Salary[121:149]
print(y)
...
completedata <- completedata %>%
  mutate(ln_Region_Weighted_Average_Salary_ncsi = ifelse(Cluster == 'Defence', NA, ln_Region_Weighted_Average_Salary_ncsi))

completedata$ln_Region_Weighted_Average_Salary_ncsi[defence_indices] <- spline_values
...
write_xlsx(completedata, "outputs/results_defence_region_weights.xlsx")
