...
only_defence <- function(oscar) {
  oscar %>%
    select(everything()) %>%
    filter(`Segment Department Long Name` == 'Ministry of Defence' | `Organisation` == 'Ministry of Defence')
}

only_defence_v2 <- function(oscar) {
  oscar %>%
    select(everything()) %>%
    filter(`Department` == 'Ministry of Defence' | `Organisation Name` == 'Ministry of Defence')
}
...
calculate_intensity_logs <- function(oscar, Num) {
  ...
  completedata <<- completedata %>% 
    mutate(admin_running_costs = ifelse(Cluster=='Defence' & Num==Observation, round((all_admin_rdel_total - all_admin_staff_total)), admin_running_costs))
  
  completedata <<- completedata %>% 
    mutate(admin_total = ifelse(Cluster=='Defence' & Num==Observation, round(all_admin_rdel_total), admin_total))
  
  completedata <<- completedata %>% 
    mutate(prog_total = ifelse(Cluster=='Defence' & Num==Observation, round(all_prog_rdel_total), prog_total))
  
  completedata <<- completedata %>% 
    mutate(admin_staff = ifelse(Cluster=='Defence' & Num==Observation, round(all_admin_staff_total), admin_staff))
  
  completedata <<- completedata %>% 
    mutate(prog_staff = ifelse(Cluster=='Defence' & Num==Observation, round(all_prog_staff_total), prog_staff))
  
  running_costs <<- log(((all_admin_rdel_total - all_admin_staff_total) / (all_admin_rdel_total + all_prog_rdel_total)) * 100)
  
  completedata <<- completedata %>%
    mutate(ln_Admin_Running_Cost_Intensity = ifelse(Cluster=='Defence' & Num==Observation, running_costs, ln_Admin_Running_Cost_Intensity))
  
  admin_staff_costs <<- log((all_admin_staff_total / (all_admin_staff_total + all_prog_staff_total)) * 100)
  
  completedata <<- completedata %>%
    mutate(ln_Admin_Staff_Intensity = ifelse(Cluster=='Defence' & Num==Observation, admin_staff_costs, ln_Admin_Staff_Intensity))
  
  total_admin_intensity <<- log((all_admin_rdel_total / (all_admin_rdel_total + all_prog_rdel_total)) * 100)
  
  completedata <<- completedata %>%
    mutate(ln_Total_Admin_Intensity = ifelse(Cluster=='Defence' & Num==Observation, total_admin_intensity, ln_Total_Admin_Intensity))
...
temp_oscar <- oscar2021b %>%
  all_rdel() %>%  # version format changed back, use original
  only_defence() %>% 
  specify_quarter(., pipeline_quarter, pipeline_year) %>% 
  all_admin_rdel() %>% 
  all_prog_rdel() %>% 
...
temp_oscar <- oscar2022b %>%
  all_rdel_v3() %>%  # version format changed back, use original
  only_defence_v2() %>% 
  specify_quarter(., pipeline_quarter, pipeline_year) %>% 
  all_admin_rdel_v2() %>% 
  all_prog_rdel_v2() %>% 
...
temp_oscar <- oscar2023b %>%
  all_rdel_v4() %>%  # version format changed again and del to TOTAL DEL, use v4
  only_defence() %>% # can go back to using v1 as standard column names are back
  specify_quarter(., pipeline_quarter, pipeline_year) %>% 
  all_admin_rdel() %>% # back to v1
  all_prog_rdel() %>% 
  all_admin_staff_costs_only() %>% 
  all_prog_staff_costs_only() 
...
temp_oscar <- oscar2024b %>%
  all_rdel_v5() %>%  # v5 for new version code
  only_defence() %>% 
  specify_quarter(., pipeline_quarter, pipeline_year) %>% 
  all_admin_rdel() %>% 
  all_prog_rdel() %>% 
...
write_xlsx(completedata, "outputs/results_defence_admin_intensities.xlsx")
