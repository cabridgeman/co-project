...
completedata <<- completedata %>%
  mutate(ln_Region_Weighted_Average_Salary = ifelse(Cluster=='Unity' & Num==observation, region_weighted_average, ln_Region_Weighted_Average_Salary))
...
x <- c(1:29)
y <- completedata$ln_Region_Weighted_Average_Salary[61:89]
print(y)
...
completedata <- completedata %>%
  mutate(ln_Region_Weighted_Average_Salary_ncsi = ifelse(Cluster == 'Unity', NA, ln_Region_Weighted_Average_Salary_ncsi))

completedata$ln_Region_Weighted_Average_Salary_ncsi[unity_indices] <- spline_values
...
write_xlsx(completedata, "outputs/results_unity_region_weights.xlsx")
