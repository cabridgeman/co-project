...
filter_defence <- function(css) {
  css <- css %>% 
    select(everything()) %>% 
    filter(Organisation %in% mod_orgs | Organisation %in% dstl_orgs | Organisation %in% deqsu_orgs | Organisation %in% ukho_orgs)
}
...
defence_fte <- read_excel("results_defence_fte_1.xlsx", sheet = 1)
defence_fte <- defence_fte %>% 
  filter(Cluster=='Defence')
...
total_fte_for_observation <- defence_fte$FTE.Total[observation]
...
completedata <<- completedata %>%
  mutate(ln_Seniority_Weighted_Average_Salary = ifelse(Cluster=='Defence' & Num==observation, seniority_weighted_average, ln_Seniority_Weighted_Average_Salary))
...
x <- c(1:29)
y <- completedata$ln_Seniority_Weighted_Average_Salary[121:149]
print(y)
...
completedata <- completedata %>%
  mutate(ln_Seniority_Weighted_Average_Salary_ncsi = ifelse(Cluster == 'Defence', NA, ln_Seniority_Weighted_Average_Salary_ncsi))

completedata$ln_Seniority_Weighted_Average_Salary_ncsi[defence_indices] <- spline_values
...
write_xlsx(completedata, "outputs/results_defence_seniority_weights.xlsx")
