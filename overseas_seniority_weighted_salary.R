...
overseas_fte <- read_excel("results_overseas_fte_1.xlsx", sheet = 1)
overseas_fte <- overseas_fte %>% 
  filter(Cluster=='Overseas')
...
total_fte_for_observation <- overseas_fte$FTE.Total[observation]
...
completedata <<- completedata %>%
  mutate(ln_Seniority_Weighted_Average_Salary = ifelse(Cluster=='Overseas' & Num==observation, seniority_weighted_average, ln_Seniority_Weighted_Average_Salary))
...
x <- c(1:29)
y <- completedata$ln_Seniority_Weighted_Average_Salary[91:119]
print(y)
...
completedata <- completedata %>%
  mutate(ln_Seniority_Weighted_Average_Salary_ncsi = ifelse(Cluster == 'Overseas', NA, ln_Seniority_Weighted_Average_Salary_ncsi))

completedata$ln_Seniority_Weighted_Average_Salary_ncsi[overseas_indices] <- spline_values
...
write_xlsx(completedata, "outputs/results_overseas_seniority_weights.xlsx")
