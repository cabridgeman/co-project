...
plot3 <- ggplot(defence, aes(x=Finance_q3_concentration_ncsi, y=ln_Admin_Staff_Intensity)) +
  geom_smooth(method='lm', colour='black', fill='lightgrey', size=1.2) +
  geom_point(shape=18, size=3.5, colour='#38802f') +
  theme_classic() +
  theme(text = element_text(family = "Times New Roman"),
        axis.title.x = element_text(size = 16, vjust = -1.4),
        axis.title.y = element_text(size = 16),
        ...
        axis.text.y = element_text(size = 13),
        legend.text = element_text(size = 12)) +
  labs(title = "Finance Profession Concentration on Admin Staff \nExpenditure Intensity in Defence Cluster \n",
       x = (x=expression(atop(bold("Finance Profession Concentration"),atop(italic("(at 75th percentile)"))))),
       y = (y=expression(atop(bold("Administrative Staff Expenditure Intensity"),atop(italic("(log transformed (base e))")))))) +
  scale_x_continuous(
    labels = function(x) paste0(formatC(x * 100, format = "f", digits = 0), "%"),  
    limits = c(0.3032178, 0.4318182),
    breaks = seq(0.30, 0.45, by = 0.01)
  )
...
#### non-corr graphs for figure 17 (or whichever number it ends up being)



plot4 <- ggplot(overseas, aes(x=Finance_q2_concentration_ncsi, y=ln_Admin_Running_Cost_Intensity)) +
  geom_smooth(method='lm', colour='black', fill='lightgrey', size=1.5, linetype='dashed') +
  geom_point(shape=18, size=4.2, colour='#002f5d') +
  theme_classic() +
  theme(text = element_text(family = "Times New Roman"),
        axis.title.x = element_text(size = 16, vjust = -1.4),
        axis.title.y = element_text(size = 16),
  ...
  scale_x_continuous(
    labels = function(x) paste0(formatC(x * 100, format = "f", digits = 0), "%"),  
    limits = c(0.16666667, 0.50000000),
    breaks = seq(0.16666667, 0.50000000, by = 0.05)
  )
