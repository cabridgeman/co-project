...
unity_filter <- function(css) {
  css <- css %>% # filtering to all unity departments and child agencies
    select(everything()) %>% 
    filter(Organisation %in% hmrc_orgs | Organisation %in% dft_orgs | Organisation %in% hclg_orgs)
}

unity_filter_v2 <- function(css) {
  css <- css %>% # filtering to all unity departments (overalls)
    select(everything()) %>% 
    filter(Organisation %in% unity_depts_overalls) # for last entry, we need to group ESNZ + DSIT together
}
...
completedata <<- completedata %>%
  mutate(FTE.Total = ifelse(Cluster=='Unity' & Num==Observation, total_fte, FTE.Total))
...
x <- c(1:29)
y <- completedata$FTE.Total[61:89]
print(y)
...
plot(x, y, main = "unity FTE Natural Cubic Spline Interpolation", xlab = "X", ylab = "Y", col = "blue", pch = 19)
lines(spline_result, col = "red")


defence_indices <- which(completedata$Cluster == 'Unity')

completedata <- completedata %>%
  mutate(fte_ncsi = NA)

completedata <- completedata %>%
  mutate(fte_ncsi = ifelse(Cluster == 'Unity', NA, fte_ncsi))
...
write_xlsx(completedata, "outputs/results_unity_fte.xlsx")
