...
Synergy_filter_v2 <- function(css) {
  css <- css %>% # filtering to all Synergy departments (overalls)
    select(everything()) %>% 
    filter(Organisation %in% synergy_depts_overalls) # for last entry, we need to group ESNZ + DSIT together
}
...
completedata <<- completedata %>%
  mutate(FTE.Total = ifelse(Cluster=='Synergy' & Num==Observation, total_fte, FTE.Total))
...
x <- c(1:29)
y <- completedata$FTE.Total[31:59]
print(y)
...
plot(x, y, main = "synergy FTE Natural Cubic Spline Interpolation", xlab = "X", ylab = "Y", col = "blue", pch = 19)
lines(spline_result, col = "red")


defence_indices <- which(completedata$Cluster == 'Synergy')

completedata <- completedata %>%
  mutate(fte_ncsi = NA)

completedata <- completedata %>%
  mutate(fte_ncsi = ifelse(Cluster == 'Synergy', NA, fte_ncsi))
...
write_xlsx(completedata, "outputs/results_synergy_fte.xlsx")
