...
completedata <<- completedata %>%
  mutate(FTE.Total = ifelse(Cluster=='Matrix' & Num==Observation, total_fte, FTE.Total))
...
plot(x, y, main = "Matrix FTE Natural Cubic Spline Interpolation", xlab = "X", ylab = "Y", col = "blue", pch = 19)
lines(spline_result, col = "red")


defence_indices <- which(completedata$Cluster == 'Matrix')

completedata <- completedata %>%
  mutate(fte_ncsi = NA)

completedata <- completedata %>%
  mutate(fte_ncsi = ifelse(Cluster == 'Matrix', NA, fte_ncsi))
...
write_xlsx(completedata, "outputs/results_matrix_fte.xlsx")
