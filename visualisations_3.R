...
defence$Date_Published <- as.Date(defence$Date_Published, format = "%Y-%m-%d")  # Adjust format as needed


defence$admin_total <- defence$admin_total / 1000
defence$admin_staff <- defence$admin_staff  / 1000
defence$admin_running_costs <- defence$admin_running_costs / 1000
...
plot1 <- ggplot(defence, aes(x = Date_Published)) +
  ...
  geom_smooth(aes(y = admin_running_costs, colour = "admin_running_costs"),
              method = 'loess', span=1/4,
              size = 1.6, se = FALSE) +
  labs(title = "Administrative Expenditure in Defence Cluster \n",
       x = '\n Year',
