...
overseas_filter <- function(css) {
  css <- css %>% # filtering to all overseas departments and child agencies
    select(everything()) %>% 
    filter(Organisation %in% fcdo_orgs | Organisation %in% fcdoservices_orgs | Organisation %in% wilton_orgs)
}

overseas_filter_v2 <- function(css) {
  css <- css %>% # filtering to all overseas departments (overalls)
    select(everything()) %>% 
    filter(Organisation %in% overseas_depts_overalls) # for last entry, we need to group ESNZ + DSIT together
}
...
completedata <<- completedata %>%
  mutate(FTE.Total = ifelse(Cluster=='Overseas' & Num==Observation, total_fte, FTE.Total))
...
x <- c(1:29)
y <- completedata$FTE.Total[91:119]
print(y)
...
plot(x, y, main = "overseas FTE Natural Cubic Spline Interpolation", xlab = "X", ylab = "Y", col = "blue", pch = 19)
...
completedata <- completedata %>%
  mutate(fte_ncsi = ifelse(Cluster == 'Overseas', NA, fte_ncsi))

completedata$fte_ncsi[overseas_indices] <- spline_values


# WRITE TO DATAFRAME

write_xlsx(completedata, "outputs/results_overseas_fte.xlsx")
